...
Page({
  ...
  downloadMsg(e){
    ...
    longUrl = 'https://qiniuoss.822644.com/' + url
    //下载文件
    let _this = this
    console.log(url)
    const downloadTask = wx.downloadFile({
      url: longUrl, //仅为示例，并非真实的资源
      success: function (res) {
  ...
  getDetail(){
    ...
    global.$ajax("/uac/auth/pcb/findPcbOrderDetail/" + id,{})
      .then(res => {
        ...
        this.setData({
          OrderStatus:str,
          payPoint: Math.ceil(this.data.contactDetail.totalPrice * 2),
  ...
  onLoad: function (options) {
    ...
    if (wx.getStorageSync("userInfo") !== null) {
      this.setData({
        userInfo: wx.getStorageSync("userInfo222").id
      })
    } else {
      this.setData({
        userInfo: undefined
