...
Page({
  ...
  onLoad: function (options) {
    console.log(options)
    this.data.goodsList = wx.getStorageSync("goodsPayInfo") != '' ? JSON.parse(wx.getStorageSync("goodsPayInfo")) : []
  ...
  onImmediateUse(e){
    ...
    if (Number.parseInt(data.couponTop) === 0) {
      ...
      if (Number.parseInt(data.couponTop) > this.data.goodsAllPrice) {
        wx.showToast({
          title: `商品价格不符合当前优惠券"满${data.couponTop}可用"的条件`,
          icon:'none'
        })
      } else {
        this.setCouponFn(data)
  ...
  setCouponFn(data) {
    ...
    this.setData({
      goodsPaymentPrice: this.data.goodsPaymentPrice,
      goodsPoints: this.data.goodsPoints,
  ...
  onBuyNow(){
    ...
    if (Object.keys(this.data.confirmOrder).length === 0) {
      ...
      let param = {
        ...
        omcGoodsOrderViEnList: JSON.parse(wx.getStorageSync('goodsPayInfo')), // 当前商品信息列表
      ...
      global.$ajax('/uac/auth/goodsOrder/createOrder', param).then(res => {
        if (res.data.code === 200) {
          if (res.data.result.code === 1) {
            ...
            wx.setStorageSync('createOrder', JSON.stringify(res.data.result)) // 点击去支付时，创建的商品订单信息
          } else {
            wx.showToast({
