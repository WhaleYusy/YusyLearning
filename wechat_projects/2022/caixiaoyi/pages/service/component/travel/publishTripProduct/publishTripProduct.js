...
Page({
  ...
  confirmStarBookDate(value) {
    console.log(value)
    var submitList = this.data.submitList;
    submitList.startTime = this.setFormat(value.detail);
    this.setData({
      timeStartComparison: value.detail,
      isShowDate1:false,
      submitList: submitList
    })
    ...
    if (
      Number.parseInt(value.detail) >
      Number.parseInt(this.data.timeEndComparison)
    ) {
      wx.showToast({
  ...
  confirmEndBookDate(value) {
    console.log(value)
    var submitList = this.data.submitList;
    submitList.endTime = this.setFormat(value.detail);
    this.setData({
      timeEndComparison: value.detail,
      isShowDate2: false,
      submitList: submitList
    })
    if (submitList.endTime == submitList.startTime) {
    ...
    if (
      Number.parseInt(value.detail) <
      Number.parseInt(this.data.timeStartComparison)
    ) {
      wx.showToast({
  ...
  toChooseRoute(event) {
    console.log(event)
    var d = event.currentTarget.dataset.item;
    var submitList = this.data.submitList;
...
//热门
  ...
  onchildHeightSel(data) {
    console.log(data)
    var submitList = this.data.submitList;
    submitList.childHeightName = data.currentTarget.dataset.item.name;
    submitList.childHeightStatus = data.currentTarget.dataset.item.status;
    this.setData({
      submitList: submitList,
      childHeightPopup:false
    })
  ...
  onRelease2() {
    ...
    } else if (submitList.tourDetDesc == "") {
      wx.showToast({
        title: "请输入图文详情",
        icon: 'none'
      })
