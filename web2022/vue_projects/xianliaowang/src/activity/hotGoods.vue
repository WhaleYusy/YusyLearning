      ...
      <ul class="list-box" >
        <li v-for="(v,k) in dataList" @click="toProInfo(v)" :key="k" :style=" ((k+1) % 4 == 0) ? 'margin-right:0' : ''" >
          <img :src="v.mainImage" alt />
			<p class="pItem">
				<span class="nowPrice" style="margin-right:0" >￥{{v.price.toFixed(3)}}元</span>
				<span class="codeCharge" >/{{(v.price * 10).toFixed(2)}}积分换购</span>
			</p>
          <div class="list-box-name" v-html="v.materialsName"></div>
        </li>
      </ul>
