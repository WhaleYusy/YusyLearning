<template>
  <div class="indexbig">
    <div class="mall_home_indedx">
      ...
      <ul class="list-box">
        <li v-for="(v,k) in dataList" :key="k" :style=" ((k+1) % 4 == 0) ? 'margin-right:0' : ''"
            @click="idleMaterialLi(v.id)">
    ...
    export default {
        ...
        methods: {
            ...
            getData(loadMore) {
                ...
                this.pageRequest.keywords = this.$route.query.keyWords
                this.$http.post(`/api-user/app/mdcidlematerials/idleConditions`, this.pageRequest).then((res) => {
            ...
            getLinkData(pageNum) {
                this.$http.post('/pc/link/getLinks', {pageNum: pageNum, pageSize: 10}).then(res => {
                    ...
                    // 友情链接 div 盒子高度
                    this.$nextTick(() => {
                        this.$refs.mallHomeLink.style.height = `${638 - localStorage.getItem('navLength') * 40}px`
                    })
                })
            },
            ...
            // 获取首页轮播广告
            getAdvers() {
                this.$http.post(`api-user/app/adv/getApiAdvList`, {positionCode: '1100'}).then((res) => {
