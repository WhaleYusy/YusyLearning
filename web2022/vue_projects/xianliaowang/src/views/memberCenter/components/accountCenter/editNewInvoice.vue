  ...
  <div class="VATInvoiceInformation_box">
    <div class="VATInvoiceInformation_top_form">
      ...
      >
        ...
        <el-form-item label>
          <el-button type="primary" size="small" @click="submitForm('formLabelAlign')">提交修改</el-button>
        </el-form-item>
      </el-form>
    </div>
  </div>
...
export default {
  ...
  data() {
    var registerPhones = (rule, value, callback) => {
      if (
        ...
        ) && !/0\d{2,3}\d{7,8}/.test(this.formLabelAlign.registerPhone)
      ) {
  ...
  methods: {
    ...
    getFindInvoiceInfo() {
      this.$http
        .post(`/pc/invoice/findInvoiceDetail/${this.datas.id}`)
        .then(res => {
          if (res.data.data.code == 1) {
            this.formLabelAlign = res.data.data.msg;
            this.formLabelAlign.consigneeAddress = this.formLabelAlign.consigneeAddress.split(
              ","
            );
          } else {
