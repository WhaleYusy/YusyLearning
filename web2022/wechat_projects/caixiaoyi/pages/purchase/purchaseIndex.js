  ...
  chooseAndClose(e){
    ...
    global.$ajax("/uac/auth/wx/common/demandTypeList",{},{type:'GET'}).then(res => {
      ...
      global.$ajax("/uac/auth/wx/common/findMaterialDemandListByClassId", {
        ...
        console.log(data)
          let arrTem = []
          arrTem = arrTem.concat(data.data.result.list)
  ...
  getFollowBox(){
    ...
    if (wx.getStorageSync('userInfo')) { //已授权用户 不弹出 2019.12.31
      ...
      if (wx.getStorageSync("loginStatus")) {  //是否已弹过弹框
        ...
        if (wx.getStorageSync('loginStatusNum') < 3) { //弹框次数未达三次
          //弹框
          // setTimeout(() => {
            this.setData({
              // isShowPopup: false,
          ...
          wx.setStorage({
            key: 'loginStatusNum',
            data: wx.getStorageSync('loginStatusNum') + 1
          })
        } else {  //已够三次，不弹
          return;
        }
