...
var inputVal = '';
var msgList = [];
var windowWidth = tt.getSystemInfoSync().windowWidth;
var windowHeight = tt.getSystemInfoSync().windowHeight;
var keyHeight = 0;
...
Page({
  ...
  onLoad: function (options) {    
    ...
    tt.setNavigationBarTitle({ title: chatTitle });
    pageThis = this;  
    ...
    // this.getShopInfo();
    socketTask = websocket.connect(wsInit)
    // wsInit();
  },
  // 获取会员等级等信息
  getUserLevel() {
  ...
  send() {
    ...
    if (sendMsg.trim() == "") {
      ...
      const sendAudioContext = tt.createInnerAudioContext();
...
function wsInit() {  
  socketTask.onMessage((res) => {
    ...
    const msnAudioContext = tt.createInnerAudioContext();
