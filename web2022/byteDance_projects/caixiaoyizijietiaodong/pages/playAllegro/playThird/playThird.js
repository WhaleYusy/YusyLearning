...
Page({
  ...
  getUserInfo() {
    global.$ajax("/uac/auth/wx/common/findUserInfo",{}).then(res => {
      ...
      tt.setStorageSync('userInfo222', res.data.result)
  ...
  onLoad: function (options) {
    ...
    //   console.log(res)
    // })
    this.data.resMsg = JSON.parse(tt.getStorageSync("res"));
    this.setData({
      resMsg: JSON.parse(tt.getStorageSync("res"))
    })
    if (this.data.resMsg !== null) {
      this.data.res = JSON.parse(tt.getStorageSync("res")).mdcPcbOrderBViEn;
      this.data.res2 = JSON.parse(tt.getStorageSync("res")).mdcPcbOrderViEn;
    }
    this.setData({
      res: this.data.res,
      res2: this.data.res2
