<template>
  <div class="indexbig">
    <div class="mall_home_indedx">
      ...
      <ul class="list-box" >
        <li v-for="(v,k) in dataList"  :key="k" :style=" ((k+1) % 4 == 0) ? 'margin-right:0' : ''" @click="purchaseLi(v.id)">
          <!-- <img :src="v.mainImage" alt /> -->
          <p class="mesgItem list-box-names" >
		      <span class="pStatus" :class="v.isEnd == 0 ? 'ing' : '' ">{{v.isEnd == 0 ? '进行中' : '已结束'}}</span>拼单:
			  <span v-html="v.materialsName"></span>
		  </p>
		  <p class="mesgItem">
      ...
      <p class="mesgItem ">
		      <span class="mesgItemTitle">交货日期：</span>
		      <span class="mesgItemContent">{{v.deliveryTime | timeFormat('yyyy-mm-dd')}}</span>
		  </p>
      <p class="mesgItem ">
		      <span class="mesgItemTitle">单价：</span>
		      <span class="mesgItemContent">{{v.price}}</span>
		  </p>
		  <p class="mesgItem ">
      ...
      <p class="mesgItem ">
		      <span class="mesgItemTitle">还剩可拼单数：</span>
		      <span class="mesgItemContent">{{v.surplusNum}}</span>
		  </p>
        </li>
      </ul>
  ...
  methods: {
		...
		getData(loadMore){
		  if(loadMore){
			...
			this.pageRequest.keywords = this.$route.query.keyWords;
			this.$http.post(`/api-user/app/mdcbuyingmaterials/buyingConditions`,this.pageRequest).then((res)=>{
    ...
    toProInfo(item) {
      // if (this.$route.query.type == "normal") {
        ...
        //   query: { item: item.goodsId, type: "normal",id:29,type:2 }
        // });
      // } else {
      //   this.$router.push({
      //     path: "/ReCommonDetail",
