     ...
     <div class="proDetailData">
      ...
      <div class="proIntroduceAndPdf" style="margin-top: 20px;">
        ...
        <div class="purchaseCommonDetailContent">
          <div class="pcdc-1">
            <span>{{ pageData.omcOrderGoodsVos[0].goodsJson.materialsName }}</span>
            <!-- <span @click="collectGoods"> -->
          </div>
          <!-- <div class="pcdc-3">地址：{{ pageData.deliveryAddress }}</div> -->
          <div class="pcdc-3">
            品牌：{{pageData.omcOrderGoodsVos[0].goodsJson.field1}}
          </div>
          <div class="pcdc-3">
            单价：{{pageData.omcOrderGoodsVos[0].unitPrice}}元/件
          </div>
          <div class="pcdc-3">拼单数量：{{ pageData.totalNum }}</div>
          <div class="pcdc-3">
            拼单时间：{{ pageData.createdTime}}
          </div>
           <div class="pcdc-3">
            预付款金额：{{ (pageData.perPay-pageData.postagePrice).toFixed(2)}}元
          </div>
          <div class="pcdc-3">
            尾款金额：{{ (pageData.totalPrice-pageData.perPay).toFixed(2)}}元
          </div>
          <div class="pcdcElse">(备注:预付{{percentInfo.perpayPercnet}}%定金{{percentInfo.platformPercent}}%的平台服务费)</div>
          <!-- <div class="pcdc-3">地址：{{pageData.deliveryAddress}}</div> -->
<!--          <div class="pcdc-3">-->
<!--            <span>收藏：</span>-->
...
<!--         <div class="purchaseCommonDetailLinkman">-->
<!--         <span class="LinkmanTitle">拼单详情:</span>MOQ<span></span>个，SOQ<span></span>个,交期8-13周。预付30%的订金，尾款货到深圳结清。订货资金由工行银行第三方监督。用户收到货后银行再把钱款转到原厂。-->
<!--         </div>-->
         <div class="purchaseBottomButton">
          <div class="purchaseCommonDetailButton" @click="contactKefu">联系客服</div>
         <div class="purchaseCommonDetailButton" v-if="pageData.status!=0&&pageData.status!=-1&&pageData.finalPayType==0" @click="startSpell">尾款支付</div>
         </div>
         <!-- 联系客服弹窗 -->
         <el-dialog :visible.sync="contactVisible" width="400px" height="400px" >
           <div class="contactContent">
             <p>微信客服</p>
            <img src="../assets/loginImg/wxkf.jpg" alt/>
            </div>
        </el-dialog>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.pageData=JSON.parse(sessionStorage.getItem("spellDetail"))
        ...
        methods: {
            startSpell(){//开始拼单
               this.$router.push({path: "../cart/payType", query: {orderNo: this.pageData.orderNo, endPayMoney:(this.pageData.totalPrice-this.pageData.perPay).toFixed(2),type:1}});
            },
            contactKefu(){
                    ...
                    .then(res => {
                        ...
                        if (this.searchType == 1) {
                          if(this.pageData.detailImages == "" || this.pageData.detailImages == null){
                            ...
                            this.pageData.detailImages = this.swiperList;
                        }
                        this.$nextTick(() => {
                            this.initSwiper1();
                        });
