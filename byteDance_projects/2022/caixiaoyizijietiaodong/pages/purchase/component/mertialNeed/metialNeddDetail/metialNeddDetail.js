...
Page({
  ...
  downLoad(e){
    ...
    return;
    //以上提示功能暂未开放 2020.01.17 因为上传文件没有视频图片类型，字节跳动目前只支持视频图片类型的下载
    ...
    global.$ajax("/uac/auth/wx/common/findRecommendMaterialDemandList", {
      ...
      .then(res => {
        ...
        if (res.data.code == 200) {
          ...
          this.setData({
            dataLists: filter.purchaseListFd(res.data.result)
          })
        }
      });
  },
   ...
   global.$ajax("/uac/auth/wx/common/findDemandEvaluate", this.data.commentList,{type:'POST'})
      .then(res => {
        if (res.data.code === 200) {
          ...
          this.setData({
            appriseList: filter.messageListFd(res.data.result.list),
  ...
  getFindDemandMessageList() {
    global.$ajax("/uac/auth/wx/demand/findDemandMessageList", this.data.leavingParam)
      .then(res => {
        ...
        if (res.data.code == 200) {
          let arr = old.concat(filter.messageListFd(res.data.result.list))
          this.setData({
            commentListData: arr,
  ...
  toShare(){
    ...
    let img = this.data.pictureAddressImg[0] || "http://qiniuoss.822644.com/wxPic/20191226/789844078442324992.jpg";

    // this.image_to_base64(img,'banner64');
    console.log(tt.getStorageSync('banner64'))
    //绘制产品图
  ...
  onLoad: function (options) {
    ...
    // 清理分享图片的缓存
    tt.removeStorageSync('storageKeyUrl');
    tt.removeStorageSync('storageKeyUrlCode');
  },
  ...
  onShareAppMessage: function () {
    ...
    let message = {
      ...
      desc: that.data.companyInfo.title,
      path: sharePath,
...
*/
  initQrCode(mtId) {
    ...
    global.$ajax('/uac/auth/wx/gen_qr_code', data, { type: 'POST' }).then((res) => {
      if (res.data.code == 200) {
        //存储二维码base64
        tt.setStorageSync('code64', res.data.result.body)
        this.setData({
          qrcodeUrl: "data:image/gif;base64,"+res.data.result.body
        })
  ...
  drawBanner(storageKey){
    storageKey = 'banner64'
      ...
      let filePath = `${tt.env.USER_DATA_PATH}/656`;
      tt.getFileSystemManager().writeFile({
        filePath: filePath,
        // this.getBase64Data()
        data: tt.base64ToArrayBuffer(tt.getStorageSync(storageKey)),
        encoding: 'binary',
        success: () => {
      ...
      return new Promise((resolve, reject) => {
        promise.then((filePath) => {
         console.log(filePath)
          console.log(that)
        var cotent = tt.createCanvasContext('myCanvas');
        //这里的 drawImage 是要干嘛
  ...
  drawCode(storageKey){
      ...
      console.log(this)
      let promise = new Promise((resolve, reject) => {
      let filePath = `${tt.env.USER_DATA_PATH}/5645`;
      tt.getFileSystemManager().writeFile({
        filePath: filePath,
        // this.getBase64Data()
        data: tt.base64ToArrayBuffer(tt.getStorageSync(storageKey)),
  ...
  addCode(typeNum) {
    ...
    let promise1 = new Promise(function (resolve, reject) {
      let saveStr = that.data.pictureAddressImg[0] || "http://qiniuoss.822644.com/wxPic/20191226/789844078442324992.jpg";
      tt.getImageInfo({
        src: saveStr,    //请求的网络图片路径
        success: function (res) {
          console.log(1)
          //请求成功后将会生成一个本地路径即res.path,然后将该路径缓存到storageKeyUrl关键字中
          tt.setStorageSync('storageKeyUrl',res.path);
          resolve(res)
        }
      })
    });
    let promise2 = new Promise(function (resolve, reject) {
      let saveStr2 = 'http://qiniuoss.822644.com/wxPic/20191227/790640962295571456.png';
      tt.getImageInfo({
        src: saveStr2,    //请求的网络图片路径
        success: function (res) {
          console.log(1)
          //请求成功后将会生成一个本地路径即res.path,然后将该路径缓存到storageKeyUrl关键字中
          tt.setStorageSync('storageKeyUrlCode',res.path);
          resolve(res)
        }
      })
    });
    Promise.all([
      ...
      console.log(res)
      //主要就是计算好各个图文的位置
      ctx.setFillStyle('#ffffff');//然后就可以赋给fillStyle了
      ctx.fillRect(0,0,this.data.canvasWidth,this.data.canvasHeight);
      //前两个参数表示矩形左上角的点的x,y坐标，w,h表示矩形的宽高。

      let img = tt.getStorageSync('storageKeyUrl')
      ...
      this.addConAndCode2();
      this.addConAndCode3();
      let imgCode = tt.getStorageSync('storageKeyUrlCode')
      ...
      tt.downloadFile({
        ...
        success(res) {
          tt.getFileSystemManager().readFile({
            ...
            success: res => { //成功的回调
              // console.log(res.data)
              tt.setStorageSync(key, res.data)
